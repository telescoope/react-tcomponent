...
function ChartDonut(props) {
  ...
  if (
    ...
    isArray(props.data) &&
    !isUndefined(props.dataValue) &&
    !isNull(props.dataValue)
  ) {
    for (var i = 0; i < props.data.length; i++) {
      if (isString(props.dataValue)) {
        series.push(props.data[i][props.dataValue])
        console.log('COND1')
      } else if (isPlainObject(props.dataValue)) {
        let isi = {}
        Object.keys(props.dataValue).map((key, index) => {
          isi[props.dataValue[key]] = props.data[i][key]
        })
        series.push(isi)
        console.log('COND2')
      } else if (isArray(props.dataValue)) {
        let isi = []
        for (var i2 = 0; i2 < props.dataValue.length; i2++) {
          isi.push(props.data[i][props.dataValue[i2]])
        }
        series.push(isi)
        console.log('COND3')
      }
  ...
  */
  ...
  if (!isUndefined(props.zoom) && !isNull(props.zoom) && isPlainObject(props.zoom)) {
  ...
  if (!isUndefined(props.fill) && !isNull(props.fill) && isPlainObject(props.fill)) {
  ...
  if (!isUndefined(props.grid) && !isNull(props.grid) && isPlainObject(props.grid)) {
